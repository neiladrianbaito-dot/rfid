...
import { Wallet, Lock, Activity, User, Phone, Tag, ShieldCheck, LogOut, PlusCircle, KeyRound, CreditCard, Mail } from "lucide-react";
          ...
          <Card className="md:col-span-1 border-slate-800 bg-slate-900/40 backdrop-blur-md border-t-emerald-500/50 border-t-2">
            <CardContent className="pt-6">
              <div className="flex justify-between items-start mb-2">
                ...
                <Button size="sm" variant="outline" onClick={() => topup.setIsOpen(true)} className="h-7 text-[10px] bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white">
                  <PlusCircle className="h-3 w-3 mr-1" /> TOP UP
                </Button>
              </div>
              <h2 className={`text-5xl font-black text-white tracking-tighter ${isPulsing ? "balance-pulse" : ""}`}>{balanceText}</h2>
              <div className="mt-3 space-y-1">
                <div className="w-full bg-slate-800 rounded-full h-1 overflow-hidden">
                  <div className={`h-1 rounded-full transition-all ${isAtMaxBalance ? "bg-red-500" : currentBalance / 20000 >= 0.8 ? "bg-amber-400" : "bg-emerald-500"}`} style={{ width: `${Math.min((currentBalance / 20000) * 100, 100)}%` }} />
                </div>
                <p className="text-[9px] text-slate-600 font-mono">
                  {isAtMaxBalance ? <span className="text-red-400/70">Max balance reached</span> : <>{"\u20B1"}{remainingTopup.toLocaleString(undefined, { minimumFractionDigits: 2 })} remaining</>}
                </p>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <Badge variant="outline" className="border-slate-700 text-slate-400 px-3 py-1">{user?.type || "Standard User"}</Badge>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-slate-800 bg-slate-900/40 backdrop-blur-md overflow-hidden">
          <CardHeader className="bg-slate-900/20 border-b border-slate-800">
            <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-400 uppercase tracking-widest">
              <Activity className="h-4 w-4 text-blue-400" />Activity Log
